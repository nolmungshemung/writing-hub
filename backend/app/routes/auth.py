...
async def login(sns_type: SnsType, user_info: UserLogin):
    if sns_type == SnsType.email:
        ...
        redis_cache.set(token=access_token['Authorization'], key='timestamp', value=timestamp_now + access_ttl_sec, ttl=access_ttl_sec)
        redis_cache.set(token=refresh_token['Authorization'], key='timestamp', value=timestamp_now + refresh_ttl_sec, ttl=refresh_ttl_sec)
        print(redis_cache.get(access_token['Authorization'], 'timestamp', access_ttl_sec))
        print(redis_cache.get(refresh_token['Authorization'], 'timestamp', refresh_ttl_sec))
        return {'Access_token': access_token['Authorization'], 'Refresh_token': refresh_token['Authorization']}
    return JSONResponse(status_code=400, content=dict(msg="NOT_SUPPORTED"))
